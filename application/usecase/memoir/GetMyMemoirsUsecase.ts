...
export class GetMyMemoirsUsecase {
    ...
    async execute(userId: string, repoId?: string, page = 1, perPage = 10): Promise<{ list: MemoirListDto[]; totalCount: number; }> {
        const [memoirs, totalCount] = await this.repo.findByUserIdPaginated(userId, repoId, page, perPage);
