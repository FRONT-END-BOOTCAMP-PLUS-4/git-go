...
export class FetchPrCommitListUsecase {
  ...
  async fetchByPullRequestNumber(repoFullName: string, prNumber: number): Promise<GithubPrCommitList[]> {
    return await this.repository.fetchByPullRequestNumber(repoFullName, prNumber);
  }
}
