...
export default function StatsPage() {
    ...
    const { data: memoirHeatmap = [] } = useQuery({
        queryKey: ["memoirHeatmap", selectedRepo?.id],
        queryFn: () =>
            fetch("/api/stats/memoir-heatmap").then((res) => res.json()),
    ...
    const {
        data: topRepos = [],
        isLoading: loadingTopRepos,
        refetch: refetchTopRepos,
    } = useQuery<{ name: string; commits: number }[]>({
        queryKey: ["topActiveRepos"],
        queryFn: () =>
            fetch("/api/stats/top-active-repos").then((res) => res.json()),
        staleTime: 1000 * 60 * 10,
    });

    const { data: statsData, isLoading: loadingStats } = useQuery({
        queryKey: ["repoStats", selectedRepo?.nameWithOwner],
        queryFn: async () => {
    ...
    }, [reloadRepoList, refetchTopRepos, resetReload]);
                ...
                <div className="flex flex-col gap-4 md:w-[30%]">
                    {loadingStats || !statsData ? (
                        ...
                        <>
                            <StatsCard
                                title="전체 커밋"
                                value={statsData.totalCommits.toString()}
                                change={statsData.commitChange}
                            />
                            <StatsCard
                                title="코드 라인 수"
                                value={statsData.totalLines.toLocaleString()}
                                change={statsData.lineChangePercent}
                            />
                            <StatsCard
                                title="작성된 회고록"
                                value={statsData.totalMemoirs.toLocaleString()}
                                change="hide"
                            />
                        </>
                    )}
